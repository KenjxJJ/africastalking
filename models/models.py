...
class Africastalking(models.Model):
    ...
    state= fields.Selection([('draft','Draft'),('sent','Sent'),('fail','fail')],default="draft",string="Status",tracking=True) 
    date_send = fields.Datetime("Time Sent",tracking=True)
    balance= fields.Char("Balance Remaining",tracking=True)
    phone_no = fields.Char("Phone Number",tracking=True)
...
class Wizard (models.TransientModel):
    ...
    def action_sending(self):
        for rec in self:
            if not rec.contact:
                raise ValidationError('Phone number missing for the Customer')
